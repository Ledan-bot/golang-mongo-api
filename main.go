...
func GetRestaurantByBorough(w http.ResponseWriter, r *http.Request) {
	ctx, _ := context.WithTimeout(context.Background(), 10*time.Second)
	w.Header().Set("Content-type", "application/json")
	params := mux.Vars(r)
	...
	cursor, err := coll.Find(context.TODO(), bson.D{{"borough", borough}})
	if err != nil {
		w.WriteHeader(http.StatusInternalServerError)
		w.Write([]byte(`{"message": "` + err.Error() + `"}`))
		return
...
func main() {
	...
	router.HandleFunc("/restaurants", GetAllRestaurants).Methods("GET")
	router.HandleFunc("/restaurants/{borough}", GetRestaurantByBorough).Methods("GET")
